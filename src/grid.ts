import { AOR, KILLBOX_ROWS, KILLBOX_COLS, fracToLatLon } from './utils'
...
export function gridLinesGeoJSON() {
  ...
  for (let c=0; c<=totalCols; c++) {
    ...
    const isKill = (c % 3 === 0)
    pushLine([[p0.lon,p0.lat],[p1.lon,p1.lat]], { kind: isKill ? 'KILLBOX' : 'KEYPAD' })
  }
  // Horizontal lines
...
export function killboxLabelsGeoJSON() {
  ...
  for (let r=0; r<totalRows; r++) {
    for (let c=0; c<totalCols; c++) {
      ...
      features.push({ type:'Feature', properties:{ label: kill }, geometry:{ type:'Point', coordinates:[p.lon,p.lat] } })
    }
  }
